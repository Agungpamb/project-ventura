...
export default function DocUpload({ 
  ...
  const handleDownloadPDFDirectly = async () => {
    ...
    try {
      ...
      setStatusMessage({
        type: 'success',
        text: `Formulir Inventarisasi untuk ${selectedRecord.CODE} berhasil diunduh!`
      });
    } catch (err: any) {
      console.error(err);
      setStatusMessage({
              ...
              <div className="p-4 rounded-xl border border-amber-500/20 bg-amber-500/5 flex flex-col md:flex-row md:items-center justify-between gap-4 animate-fadeIn">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-slate-400 mt-1 leading-relaxed">
                      Format resmi 4 halaman (Data Jaringan, Pihak Berhak, Lahan, Bangunan, Tanaman, Denah/Peta Sketsa Bidang, & Dokumentasi Foto).
                    </p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2 self-end md:self-center">
                  <button
                    ...
                    disabled={isDownloadingPDF || isSavingPDFToDrive}
                    className={`px-3.5 py-2 text-xs font-bold rounded-xl border flex items-center gap-1.5 transition-all cursor-pointer shadow-xs active:scale-98 ${
                      isDownloadingPDF
