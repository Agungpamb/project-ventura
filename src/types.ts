...
export const recordToRow = (record: LandRecord): string[] => {
  // Ensure CODE is always populated
  let recordCode = (record.CODE || "").trim();
  if (!recordCode) {
    ...
    if (cleanDesa && cleanSpan && cleanNobid) {
      recordCode = `${cleanDesa}_${cleanSpan}_${cleanNobid}`;
    }
  }
...
export interface ActivityLog {
  ...
  operatorName?: string;
  userRole: string;
  actionType: 'CREATE' | 'UPDATE' | 'UPLOAD' | 'QC' | 'ESDM_APPROVAL';
  details: string;
  recordCode: string;
}
